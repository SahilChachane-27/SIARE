                ...
                <div className="space-y-8">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div className="relative">
                      ...
                      <Input placeholder="Search by Journal Name" className="pl-12 h-14 border-primary/10 rounded-xl bg-secondary/5" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                    </div>
                    <div className="relative">
                      <Building2 className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
                      <Input placeholder="University Name" className="pl-12 h-14 border-primary/10 rounded-xl bg-secondary/5" value={universityQuery} onChange={(e) => setUniversityQuery(e.target.value)} />
                    </div>
                    <div className="relative">
                      <Hash className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
                      <Input placeholder="ISSN Number" className="pl-12 h-14 border-primary/10 rounded-xl bg-secondary/5" value={issnQuery} onChange={(e) => setIssnQuery(e.target.value)} />
                    </div>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <Select value={selectedCategory} onValueChange={setSelectedCategory}>
                      <SelectTrigger className="h-14 border-primary/10 rounded-xl bg-secondary/5"><SelectValue placeholder="Field of Study" /></SelectTrigger>
                      <SelectContent>
                        <SelectItem value="All">All Fields</SelectItem>
                        {categories.map(cat => <SelectItem key={cat.name} value={cat.name}>{cat.name}</SelectItem>)}
                      </SelectContent>
                    </Select>
                    <Button className="h-14 bg-primary text-white rounded-full font-bold shadow-xl md:col-span-2 uppercase tracking-widest">Search Catalog</Button>
                  </div>
                </div>
              ) : <div className="h-40 flex items-center justify-center font-bold text-primary/40 tracking-widest uppercase">Initializing Sources...</div>}
            </div>
          </div>
        </section>
